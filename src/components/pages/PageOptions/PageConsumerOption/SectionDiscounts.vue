<template>
	<div class="section-discounts-container">
		<div class="autopay">
			<input-switch
				v-bind="{ label: 'Autopay?', value: autopay }"
				v-on:toggle="toggle('autopay')"
			/>
		</div>
		<div class="visaCard">
			<input-switch
				v-bind="{ label: 'Visa Card?', value: visaCard }"
				v-on:toggle="toggle('visaCard')"
			/>
		</div>
		<div class="common-discounts">
			<div @click="setCommonDiscount(false)">
				<input
					type="radio"
					name="commonDiscounts"
					id="noCommonDiscount"
					:checked="noCommonDiscount"
				/>
			...
			<div v-if="isUnlimited" @click="setCommonDiscount('militaryNew')">
				<input
					...
					:checked="militaryNew"
				/>
				<label for="militaryNew"
					>Military Discount (after 8/5/19)</label
				>
			</div>
			<div v-if="isUnlimited" @click="setCommonDiscount('militaryOld')">
				<input
					...
					:checked="militaryOld"
				/>
				<label for="militaryOld"
					>Military Discount (before 8/5/19)</label
				>
			</div>
			<div v-if="isUnlimited" @click="setCommonDiscount('responderNew')">
				<input
					...
					name="commonDiscounts"
					id="responderNew"
					:checked="responderNew"
				/>
				<label for="responderNew"
					>First Responder Discount (after 8/5/19)</label
				>
			</div>
			<div v-if="isUnlimited" @click="setCommonDiscount('responderOld')">
				<input
					...
					name="commonDiscounts"
					id="responderOld"
					:checked="responderOld"
				/>
				<label for="responderOld"
					>First Responder Discount (before 8/5/19)</label
				>
			</div>
			...
			<div
				...
				"
				@click="setCommonDiscount('militaryNew')"
			>
				<input
					type="radio"
					name="commonDiscounts"
			...
			<div
				v-if="
					!isUnlimited &&
						...
						!ifLoyalty55MixAndMatch2020
				"
				@click="setCommonDiscount('responderNew')"
			>
				<input
					type="radio"
					name="commonDiscounts"
		...
		<div class="percentDiscount">
			<input-dropdown
				...
				v-on:value-changed="
					mutate({ property: 'discount', with: $event })
				"
			/>
		</div>
	</div>
	...
	computed: {
		...mapState({
			...
			isUnlimited(state, getters) {
				return getters[
					`consumer/${this.$route.params.vuexModule}/isUnlimited`
				];
			},
			noCommonDiscount(state, getters) {
				return getters[
					`consumer/${this.$route.params.vuexModule}/noCommonDiscount`
				];
			}
		}),
		ifLoyalty55MixAndMatch2020() {
		...
		ifLoyalty55OrLoyaltyGo() {
			return (
				this.chosenPlan === "Loyalty 55+" ||
				this.chosenPlan === "Loyalty Go"
			);
		},
		if110UnlimitedOr65Unlimited() {
			return (
				this.chosenPlan === "$110 Unlimited" ||
				this.chosenPlan === "$65 Unlimited"
			);
		}
	},
	methods: {
		...mapMutations({
			...
			toggle(commit, property) {
				return commit(
					`consumer/${this.$route.params.vuexModule}/toggle`,
					property
				);
			},
