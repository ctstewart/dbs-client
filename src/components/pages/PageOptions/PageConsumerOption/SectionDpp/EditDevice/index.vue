		...
		<div class="device-edit">
			<div class="dpp-total">
				...
				<span
					>${{
						(
							dppObject.fullRetail / dppObject.dppLength / 100 -
							dppObject.totalCredits / dppObject.dppLength / 100
						).toFixed(2)
					}}
					a month</span
				>
			...
			<div class="input-group">
				...
				<div class="toggle">
					<input
						type="radio"
						id="auto"
						name="deviceNameManual"
						value="false"
						v-model="deviceNameManual"
					/>
					<label
						:class="[
							deviceNameManual ? 'text-inactive' : 'text-active'
						]"
						for="auto"
						@click="toggleManualMode(dppIndex, false, [24])"
						>Auto</label
					>
					<input
						type="radio"
					...
					<label
						:class="[
							deviceNameManual ? 'text-active' : 'text-inactive'
						]"
						for="manual"
						@click="toggleManualMode(dppIndex, true, [24, 30])"
						>Manual</label
					>
				</div>
			...
			<div
				class="input-group"
				v-if="dppObject.dppLengthOptions.length > 1"
			>
				<label for="deviceName">DPP Length</label>
				<div class="toggle">
					<div
						class="toggle"
						v-for="(i, index) in dppObject.dppLengthOptions"
						:key="index"
					>
						<input
							type="radio"
						...
						<label
							:class="[
								i === dppObject.dppLength
									? 'text-active'
									: 'text-inactive'
							]"
							:for="'dppLength' + i"
							@click="
								mutateDppLength({ index: dppIndex, value: i })
							"
							>{{ i }} Month</label
						>
					</div>
			...
			<div class="input-group">
				...
				<div class="toggle">
					<input
						...
						name="dppTotalMode"
						value="false"
						v-model="dppTotalMode"
					/>
					<label
						:class="[
							dppTotalMode ? 'text-inactive' : 'text-active'
						]"
						for="dppMonthly"
						@click="dppTotalMode = false"
						>Monthly</label
					>
					<input
						type="radio"
					...
					<label
						:class="[
							dppTotalMode ? 'text-active' : 'text-inactive'
						]"
						for="dppTotal"
						@click="dppTotalMode = true"
						>Total</label
					>
				</div>
				<input
					...
					@change="
						mutateDpp({
							index: dppIndex,
							value:
								$event.target.value * dppObject.dppLength * 100
						})
					"
			...
			<div class="input-group">
				...
				<div class="toggle">
					<input
						type="radio"
						id="creditsMonthly"
						name="creditsTotalMode"
						value="false"
						v-model="creditsTotalMode"
					/>
					<label
						:class="[
							creditsTotalMode ? 'text-inactive' : 'text-active'
						]"
						for="dppMonthly"
						@click="creditsTotalMode = false"
						>Monthly</label
					>
					<input
						type="radio"
					...
					<label
						:class="[
							creditsTotalMode ? 'text-active' : 'text-inactive'
						]"
						for="creditsTotal"
						@click="creditsTotalMode = true"
						>Total</label
					>
				</div>
				<input
			...
			<div class="buttons">
				<button class="buttons__save" @click="closeEditDpp(dppIndex)">
					SAVE
				</button>
				<button
					class="buttons__delete"
					@click="removeDppValue(dppIndex)"
				>
					DELETE
				</button>
			</div>
	...
	methods: {
		...mapMutations({
			...
			mutateCredits(commit, payload) {
				return commit(
					`consumer/${this.$route.params.vuexModule}/mutateCredits`,
					payload
				);
			},
			...
			mutateDppLength(commit, payload) {
				return commit(
					`consumer/${this.$route.params.vuexModule}/mutateDppLength`,
					payload
				);
			},
			removeDppValue(commit, payload) {
				this.componentKey += 1;
				return commit(
					`consumer/${this.$route.params.vuexModule}/removeDppValue`,
					payload
				);
			},
			closeEditDpp(commit, index) {
				return commit(
					`consumer/${this.$route.params.vuexModule}/closeEditDpp`,
					index
				);
			}
		}),
